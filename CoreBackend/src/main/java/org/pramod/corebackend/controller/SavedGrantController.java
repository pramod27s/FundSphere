 ...
 *
 * GET    /api/saved-grants            -> list current user's saved grants
 * POST   /api/saved-grants/{grantId}  -> bookmark a grant (idempotent)
 * DELETE /api/saved-grants/{grantId}  -> remove a bookmark
 * GET    /api/saved-grants/ids        -> lightweight list of saved grant IDs
 */
...
public class SavedGrantController {
    ...
    @GetMapping
    public ResponseEntity<List<GrantResponse>> list(@AuthenticationPrincipal UserPrincipal principal) {
    ...
    public ResponseEntity<List<Long>> listIds(@AuthenticationPrincipal UserPrincipal principal) {
        List<Long> ids = savedGrantService.listSaved(requireUserId(principal)).stream()
                .map(GrantResponse::getId)
                .toList();
        return ResponseEntity.ok(ids);
    }

    @PostMapping("/{grantId}")
    public ResponseEntity<GrantResponse> save(@AuthenticationPrincipal UserPrincipal principal,
